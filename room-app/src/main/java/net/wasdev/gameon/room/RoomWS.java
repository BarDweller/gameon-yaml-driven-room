...
public class RoomWS extends Endpoint {
    ...
    public void receiveMessage(String message, Session session) throws IOException {
        Log.log(Level.FINE, this, "ROOMX: [{0}:{1}] sess[{2}:{3}] : {4}", this.hashCode(),this.room.getId(),session.hashCode(),session.getId(),message);
        ...
        Log.log(Level.SEVERE, this, "Unknown Message Type {0} for room {1} message {2}", contents[0], room.getId(),message);
    }
    ...
    private void addNewPlayer(Session session, String json) throws IOException {
        ...
        Log.log(Level.INFO, this, "*** Adding player {0} from room {1} via session {2}", userid,room.getId(),session.getId());
    ...
    private void removePlayer(Session session, String json) throws IOException {
        JsonObject msg = Json.createReader(new StringReader(json)).readObject();
        String userid = Message.getValue(msg.get(Constants.USERID));
        Log.log(Level.INFO, this, "*** Removing player {0} from room {1} via session {2}", userid,room.getId(),session.getId());

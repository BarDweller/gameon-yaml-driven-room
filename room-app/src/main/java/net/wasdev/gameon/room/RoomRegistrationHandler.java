...
import net.wasdev.gameon.room.LifecycleManager.RegistrationRoom;
...
public class RoomRegistrationHandler {
  ...
  private final RegistrationRoom room;
  private AtomicBoolean handling503 = new AtomicBoolean(false);
  private final String token;

  private final Client cleanClient;

  RoomRegistrationHandler(RegistrationRoom room, String id, String secret) {
    this.id = id;
    this.room = room;
    this.secret = secret;
    ...
    if (endPoint == null) {
      throw new IllegalStateException("The environment variable " + Constants.ENV_ROOM_SVC + " was not defined.");
    }
    mapLocation = System.getenv(Constants.ENV_MAP_SVC);
    if (mapLocation == null) {
      throw new IllegalStateException("The environment variable " + Constants.ENV_MAP_SVC + " was not defined.");
    }
    mapHealth = System.getenv(Constants.ENV_MAP_HEALTH_SVC);
    if (mapLocation == null) {
      throw new IllegalStateException("The environment variable " + Constants.ENV_MAP_HEALTH_SVC + " was not defined.");
    }

    token = "";

    cleanClient = ClientBuilder.newBuilder().property("com.ibm.ws.jaxrs.client.ssl.config", "DefaultSSLSettings")
        .property("com.ibm.ws.jaxrs.client.disableCNCheck", true).build();
  }

  private static class RegistrationResult {
    enum Type {
      NOT_REGISTERED, REGISTERED, SERVICE_UNAVAILABLE
    };
  ...
  private static RegistrationResult MAP_UNAVAILABLE = new RegistrationResult(
      RegistrationResult.Type.SERVICE_UNAVAILABLE);
  ...
  private Client getClient() throws Exception {
    ...
    // Build client that uses the SSLContext defined by 'DefaultSSLSettings' in
    // server.xml
    Client client = ClientBuilder.newBuilder().property("com.ibm.ws.jaxrs.client.ssl.config", "DefaultSSLSettings")
        .property("com.ibm.ws.jaxrs.client.disableCNCheck", true).build();
  ...
  private RegistrationResult checkExistingRegistration() throws Exception {
    ...
    try {
      ...
      WebTarget target = queryRoot.queryParam("owner", id).queryParam("name", room.getId());
      Response r = null;

      System.out.println("RoomRegistration checking room " + room.getId() + " at url " + target.getUri());
      ...
      case 200: {
        ...
        Invocation.Builder builder = lookup.resolveTemplate("roomId", roomId).path("{roomId}")
            .request(MediaType.APPLICATION_JSON);
        Response response = builder.get();
        respString = response.readEntity(String.class);

        Log.log(Level.FINE, this, "EXISTING_INFO({0})({1}):{2}", id, room.getId(), respString);
  ...
  private void handle503() throws Exception {
    try {
      Log.log(Level.INFO, this, "Scheduling room {0} to be registered via bg thread.", room.getId());
  ...
  private class RegisterRunnable implements Runnable {
    ...
    public void run() {
      try {
        Log.log(Level.INFO, this, "Registration thread for room {0} has awoken.", room.getId());
  ...
  public boolean performRegistration() throws Exception {
    ...
    case REGISTERED: {
      ...
      if (updatedRegistration.type == RegistrationResult.Type.REGISTERED) {
        ...
        Log.log(Level.SEVERE, this, "Unable to update room registration for room {0}", room.getId());
...
//                Log.log(Level.FINER, this, "Added exit {0} to {1} : {2}", e.getKey(), room.getId(), exit);
//            }catch(Exception ex){
...
//        room.setExits(exitMap);
  ...
  private RegistrationResult compareRoomAndUpdateIfRequired(JsonObject registeredRoom) throws Exception {
    ...
    if (room.getId().equals(info.getString("name")) && room.getName().equals(info.getString("fullName"))) {
      ...
      if (connectionDetails != null) {
        if ("websocket".equals(connectionDetails.getString("type"))
            && getEndpointForRoom().equals(connectionDetails.getString("target")) && token.equals(existingToken)) {
    ...
    if (needsUpdate) {
      System.out.println("Update required for " + room.getId());
      Log.log(Level.INFO, this, "Update required for {0}", room.getId());
      return updateRoom(registeredRoom.getString("_id"));
    } else {
      Log.log(Level.INFO, this, "Room {0} is still up to date in Map, no update required.", room.getId());
  ...
  private boolean mapIsHealthy() throws Exception {
    try {
      Response response = cleanClient.target(mapHealth).request(MediaType.APPLICATION_JSON).get();
      response.close();
  ...
  private RegistrationResult registerOrUpdateRoom(Mode mode, String roomId) throws Exception {
    ...
    registrationPayload.add("name", room.getId());
    registrationPayload.add("fullName", room.getName());
    registrationPayload.add("description", "A YAML Room");
    ...
    try {
      ...
      case UPDATE: {
        Invocation.Builder builder = root.path("{roomId}").resolveTemplate("roomId", roomId)
            .request(MediaType.APPLICATION_JSON);
    ...
    try {
      ...
      if ((mode.equals(Mode.REGISTER) && Status.CREATED.getStatusCode() == response.getStatus())
        ...
        Log.log(Level.INFO, this, "Sucessful registration/update operation against ({0})({1})({2}) : {3}", roomId, id,
            room.getId(), regString);
      } else {
        String resp = response.readEntity(String.class);

        Log.log(Level.SEVERE, "Error registering room provider : {0} : status code {1} : response {2}", room.getName(),
            response.getStatus(), String.valueOf(resp));

        r.type = RegistrationResult.Type.NOT_REGISTERED;

        throw new Exception("Room operation did not report success, got error code " + response.getStatus() + " "

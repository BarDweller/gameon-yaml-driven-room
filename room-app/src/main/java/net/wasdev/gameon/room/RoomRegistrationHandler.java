...
public class RoomRegistrationHandler {
    ...
    RoomRegistrationHandler(RoomEngine room, String id, String secret){
        this.id=id;
        this.room=room;
        this.secret=secret;
...
//        room.setExits(exitMap);
    ...
    private RegistrationResult compareRoomAndUpdateIfRequired(JsonObject registeredRoom) throws Exception{
        ...
        if(   room.getId().equals(info.getString("name"))
           && room.getName().equals(info.getString("fullName"))
                )
        {
    ...
    private RegistrationResult registerOrUpdateRoom(Mode mode, String roomId) throws Exception{
        ...
        try {
            ...
            if ( (mode.equals(Mode.REGISTER) && Status.CREATED.getStatusCode() == response.getStatus()) ||
                ...
                Log.log(Level.INFO,this,"Sucessful registration/update operation against ({0})({1})({2}) : {3}",roomId,id,room.getId(),regString);
            } else {
                String resp = response.readEntity(String.class);

                Log.log(Level.SEVERE, "Error registering room provider : {0} : status code {1} : response {2}", room.getName(), response.getStatus(), String.valueOf(resp));

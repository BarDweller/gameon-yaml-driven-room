...
public class ConditionParser {
  ...
  // left to right parser, aggregates each part into the lhs of whatever comes
  // next.
  private Expression parse(State state) throws ParseException {
        ...
        case ' ': {
          ...
          if (currentToken != null) {
            if (eval.lhs == null) {
              ...
              if (result == null) {
                ...
                if (((AndExpression) result).b == null) {
                  ...
                  throw new ParseException(
                      "ERROR: expression [" + state.expression + "] unexpected evaluation after && usage in "
                          + state.expression + " Eg, you can't do 'a==b && c==d e==f'");
                }
              ...
              throw new ParseException(
                  "ERROR: expression [" + state.expression + "] malformed. You can't do 'a==1 b==2'");
            }
          }
          break;
        }
        case '=': {
          ...
          if (state.expression.length() > (state.idx)) {
            if (state.expression.charAt(state.idx + 1) == '=') {
              if (eval.lhs == null) {
                throw new ParseException(
                    "ERROR: expression [" + state.expression + "] missing lhs for ==, Eg, you cannot do '==b'");
              } else {
                // found==
                eval.operator = "==";
                state.idx++;
              ...
              throw new ParseException(
                  "ERROR: expression [" + state.expression + "] used = but not == Eg. 'a=1'. Expected '=' but found' "
                      + state.expression.charAt(state.idx + 1) + "'");
            }
          } else {
            throw new ParseException(
                "ERROR: expression [" + state.expression + "] unmatched = at end of expression. Eg 'a='");
          }
          break;
        }
        case '!': {
        ...
        case '&': {
          ...
          if (state.expression.length() > (state.idx)) {
            if (state.expression.charAt(state.idx + 1) == '&') {
              if (result == null) {
                System.out.println(
                    "ERROR: expression [" + state.expression + "] missing lhs for &&, Eg you cannot do '&& a==b'");
              } else {
                // found &&
              ...
              throw new ParseException("ERROR: expression [" + state.expression
                  + "] used & but not && Eg. 'a==1 & b==2'. Expected '&' but found' "
                  + state.expression.charAt(state.idx + 1) + "' ");
            }
          } else {
            throw new ParseException(
                "ERROR: expression [" + state.expression + "] unmatched & at end of expression. Eg 'a==1 &'");
          }
          break;
        }
        case '|': {
    ...
    if (inQuote)
      throw new ParseException("ERROR: expression [" + state.expression
          + "]  unbalanced quote, parser ended up still inside quoted string at end of expression");
    return result;
  ...
  private String substituteVars(String exp, Map<String, Object> stateById, String args, String playerId,
      String playerName) {
    ...
    try {
      ...
      System.err.println(fixed.length());
      e.printStackTrace();
      throw e;
    }
  }

  // duplicate an expression, but swap template values for their current values
  // from state.
  private Expression copyExpressionAndFillInVars(Expression expression, Map<String, Object> stateById, String args,
      String playerId, String playerName) throws ParseException {
    if (expression == null) {
      throw new ParseException(
          "ERROR: Internal: Badly parsed expression resulted in null discovered during template resolution.");
    }
    if (expression instanceof Evaluation) {
      ...
      if (e.lhs.contains("{")) {
        throw new ParseException(
            "ERROR: Unable to satisfy all template vars requested in expression. Remaining: " + e.lhs);
      }
      if (e.rhs.contains("{")) {
        throw new ParseException(
            "ERROR: Unable to satisfy all template vars requested in expression. Remaining: " + e.rhs);
      }
    ...
    throw new ParseException(
        "ERROR: Internal: Unknown Expression Subclass " + expression.getClass().getCanonicalName());
  }

  // evaluate an expression using current state.
  public boolean evaluate(String expression, Map<String, Object> stateById, String args, String playerId,
      String playerName) {
    if (args == null)
      args = "";
